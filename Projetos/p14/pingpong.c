...
void disk_politic_fcfs();
void disk_politic_sstf();
void disk_politic_cscan();
void print_diskdriver_info();
...
void update_queues(task_t* task, int new_state)
{
    ...
    if(task == NULL)
        return;
    if(task->state == DISK_REQUEST)
        queue_rm = &__queue_disk_tasks;
    ...
    if(new_state == DISK_REQUEST)
        queue_add = &__queue_disk_tasks;
...
void func_disk_manager()
{
    while(1)
    {
        ...
        if(disk_cmd(DISK_CMD_STATUS, 0, NULL) == DISK_STATUS_IDLE)
        {
            ...
            {
                // politica de acesso ao disco
                if(DISK_POLITIC == FCFS)
                    disk_politic_fcfs();
                else if(DISK_POLITIC == SSTF)
                    disk_politic_sstf();
                else if(DISK_POLITIC == CSCAN)
                    disk_politic_cscan();
                task_t* task = (task_t*)__queue_disk_tasks;
                
                // atualiza variaveis do driver e faz chamada de disco
                __disk_driver.last_activation = systime();
                int dist = __disk_driver.last_disk_block - task->disk_block;
                if(dist < 0)
                    dist = -dist;
                __disk_driver.total_blocks += dist;
                disk_cmd(task->disk_oper, task->disk_block, task->disk_buffer);
                __disk_driver.last_disk_block = task->disk_block;
            }
        }
...
// Politica de disco FCFS (First come first served)
void disk_politic_fcfs()
{
    // fila ja esta "ordenada", nao faz nada
}

// Politica de disco SSTF (Shortest seek time first)
void disk_politic_sstf()
{
    if(__queue_disk_tasks == NULL)
        return;
    
    // seleciona primeiro a primeira tarefa da fila e calcula sua distancia
    task_t* task = (task_t*)__queue_disk_tasks;
    int dist = task->disk_block - __disk_driver.last_disk_block;
    if(dist < 0)
        dist = -dist;

    task_t* aux;

    // percorre tarefas a espera do disco
    for(aux = (task_t*)__queue_disk_tasks->next; aux != (task_t*)__queue_disk_tasks; aux = aux->next)
    {
        // ve distancia da tarefa para o bloco atual do disco
        int dist_aux = aux->disk_block - __disk_driver.last_disk_block;
        if(dist_aux < 0)
            dist_aux = - dist_aux;
        // se a distancia for menor, atualiza qual deve ser a 
        // tarefa a assumir o disco
        if(dist_aux < dist)
        {
            dist = dist_aux;
            task = aux;
    ...
    // rearranja fila para tarefa selecionada ser aquela a primeira
    __queue_disk_tasks = (queue_t*)task;
}

// Politica de disco CSCAN (Circular Scan)
void disk_politic_cscan()
{
    if(__queue_disk_tasks == NULL)
        return;
    
    int numBlocks = disk_cmd(DISK_CMD_DISKSIZE, 0, NULL);

    // seleciona primeiro a primeira tarefa da fila e calcula sua distancia
    task_t* task = (task_t*)__queue_disk_tasks;
    int dist = task->disk_block - __disk_driver.last_disk_block + numBlocks;
    dist %= numBlocks;
    ...
    for(aux = (task_t*)__queue_disk_tasks->next; aux != (task_t*)__queue_disk_tasks; aux = aux->next)
    {
        // ve distancia circular da tarefa para o bloco atual do disco
        int dist_aux = aux->disk_block - __disk_driver.last_disk_block + numBlocks;
        dist_aux %= numBlocks;

        // se a distancia circular for menor, atualiza qual deve ser a 
        // tarefa a assumir o disco
        if(dist_aux < dist)
        {
            dist = dist_aux;
...
// Imprime informacoes do driver do disco 
void print_diskdriver_info()
{
    printf("Disk driver informations: disk politic %d, total blocks %u, total usage time %u ms\n", 
        DISK_POLITIC ,__disk_driver.total_blocks, __disk_driver.total_usage_time);
}
...
void task_exit (int exitCode)
{
    ...
    if(__curr_task != &__task_dispatcher)
        task_switch(&__task_dispatcher);
    else // se eh dispatcher
    {
        // imprime informacoes do driver de disco
        print_diskdriver_info();
    }
}
...
int diskdriver_init (int *numBlocks, int *blockSize)
{
    ...
    __disk_driver.total_blocks = 0;
    __disk_driver.total_usage_time = 0;
    __disk_driver.last_activation = 0;
    __disk_driver.last_disk_block = 0;
